...
describe("HegicBoundigCurve", async () => {
  ...
  describe("ETH Bonding Curve", () => {
    it.skip("StartPrice/StartCoff", async () => {
    ...
    it("Buy HEGIC", async () => {
      ...
      expect(amountBuy).to.be.eq(
        ethers.utils.formatUnits(
          startBalanceAlice
            ...
            .sub(Txn_Inf.gasUsed.mul(Tx.gasPrice))
            .toString(),
        ),
      )
      ...
      let amountBuy1 = ethers.utils.formatUnits(
        await EthBondingCurve.s(
          await EthBondingCurve.soldAmount(),
          (await EthBondingCurve.soldAmount()).add(
            ethers.utils.parseUnits(`${amountTokens}`),
          ),
        ),
      )
      ...
      expect(amountBuy1).to.be.eq(
        ethers.utils.formatUnits(
          startBalanceAlice1
            .sub(await alice.getBalance())
            .sub(Txn_Inf1.gasUsed.mul(Tx1.gasPrice))
            .toString(),
        ),
      )
    })
    ...
    it("Sell HEGIC", async () => {
      ...
      let prc10 = ethers.utils.formatUnits(
        (
          await EthBondingCurve.s(
            (await EthBondingCurve.soldAmount()).sub(
              BN.from(ethers.utils.parseUnits(`${amountTokens}`)),
            ),
            await EthBondingCurve.soldAmount(),
          )
        )
      ...
      expect(
        ethers.utils.formatUnits(
          (await alice.getBalance())
            ...
            .add(Txn_Inf.gasUsed.mul(Tx.gasPrice))
            .toString(),
        ),
      ).to.be.eq(prc90)
