...
describe("HegicPool", async () => {
  ...
  describe("constructor & settings", async () => {
    it("should set all initial state", async () => {
      ...
      expect(await hegicPool.lockupPeriod()).to.be.eq(BN.from(2592000))
      expect(await hegicPool.lockedAmount()).to.be.eq(BN.from(0))
      expect(await hegicPool.totalShare()).to.be.eq(BN.from(0))
      expect(await hegicPool.totalBalance()).to.be.eq(BN.from(0))
  ...
  describe("setLockupPeriod", async () => {
    ...
    it("should revert if the period is greater than 60 days", async () => {
      ...
      ).to.be.revertedWith("The lockup period is too long")
    })

    it("should set the lockupPeriod correctly", async () => {
      const lockupPeriodBefore = await hegicPool.lockupPeriod()
      expect(lockupPeriodBefore).to.equal(BN.from(2592000))
      await hegicPool.setLockupPeriod(5183001)
      const lockupPeriodAfter = await hegicPool.lockupPeriod()
      expect(lockupPeriodAfter).to.be.eq(BN.from(5183001))
    })
  })
  ...
  describe("sellOption", async () => {
    ...
    it("should create an option correctly", async () => {
      ...
      const lockedForCall = amount.mul(20).div(100)
      const option = await hegicPool.options(BN.from(0))
  ...
  describe("exercise", async () => {
    ...
    it("should set values correctly", async () => {
      ...
      expect(lockedAmountBefore).to.eq(BN.from(2000))
